...
class TradingBotGUI:
    ...
    def create_main_window(self) -> None:
        ...
        try:
            ...
            # Build tabs exactly like bobot2.py
            self._build_dashboard()
            self._build_strategy_tab()  # CRITICAL: Pre-start settings
            self._build_calculator_tab()
            self._build_log_tab()
            
            # Initialize strategy parameters storage (exact bobot2.py match)
...
Created with ❤️ for professional traders"""
    ...
    # CRITICAL: Add bobot2.py GUI methods for parameter retrieval
    ...
    def _export_logs_csv(self):
        ...
        try:
            ...
            filename = f"trading_logs_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
            
            # Mock export for demonstration
            with open(filename, 'w', newline='') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow(['Timestamp', 'Level', 'Message'])
                writer.writerow([datetime.now().strftime('%Y-%m-%d %H:%M:%S'), 'INFO', 'Log export completed'])
